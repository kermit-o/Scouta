"""
Async LLM Planning Agent - Consulta REAL y ASÍNCRONA a DeepSeek para planificación
"""
import json
import asyncio
from services.robust_deepseek_client import RobustDeepSeekClient

class AsyncLLMPlanningAgent:
    def __init__(self):
        self.client = RobustDeepSeekClient()
        print("[AsyncLLMPlanningAgent] Initialized - CONSULTA ASÍNCRONA A DEEPSEEK")
    
    async def run(self, project_spec: dict) -> dict:
        """Consulta ASÍNCRONA al LLM para crear plan de desarrollo"""
        print("📊 Async LLM Planning Agent - Consultando DeepSeek para planificación...")
        ...
        try:
            ...
            plan_response = await self.client.generate_code(prompt, "Planificación de Desarrollo")
        ...
        try:
            if '{' in llm_response and '}' in llm_response:
                ...
                return {
                    ...
                    "llm_plan": llm_response[:500] + "..." if len(llm_response) > 500 else llm_response,
                    "status": "planned_by_llm"
                }
        except Exception as e:
            print(f"⚠️ No se pudo parsear plan LLM: {e}")
