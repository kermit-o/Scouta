...
from core.settings import ARTIFACT_ROOT
...
class PackagerAgent(AgentBase):
    ...
    def run(self, project_id: uuid.UUID, current_requirements: Dict[str, Any]) -> Dict[str, Any]:
        workdir = current_requirements.get("workdir") if "current_requirements" in locals() or "current_requirements" in globals() else None
        if workdir:
            from core.settings import ARTIFACT_ROOT
            from pathlib import Path
            project_dir = Path(workdir)
            output_dir = ARTIFACT_ROOT / "delivery"
        ...
        workdir = current_requirements.get("workdir")
        if workdir:
            project_dir = Path(workdir)  # PRIORIDAD NUEVA (no rompe lo existente)
        else:
            # comportamiento anterior
            project_dir = Path(workdir) if ('workdir' in locals() and workdir) else GENERATED_ARTIFACTS_ROOT / project_id_str
