"""
Async LLM Builder Agent - Consulta REAL y ASÍNCRONA a DeepSeek para GENERAR CÓDIGO
"""
import os
import uuid
import json
...
class AsyncLLMBuilderAgent:
    def __init__(self):
        self.client = RobustDeepSeekClient()
        self.generated_files = []
    
    async def run(self, project_id: str, development_plan: dict) -> dict:
        """Consulta ASÍNCRONA al LLM para GENERAR CÓDIGO de cada archivo"""
        print("🏗️ Async LLM Builder Agent - Consultando DeepSeek para GENERAR CÓDIGO...")
        
        try:
            # Crear directorio del proyecto
            project_name = development_plan.get('project_name', 'proyecto-llm').replace(' ', '-').lower()
            project_path = f"generated_projects/async-llm-{project_name}-{project_id[:8]}"
            os.makedirs(project_path, exist_ok=True)
            
            print(f"📁 Proyecto Async LLM: {project_path}")
            
            # GENERAR CÓDIGO consultando ASÍNCRONAMENTE al LLM para CADA archivo
            files_created = await self._generate_code_with_llm(project_path, development_plan)
            ...
            return {
                ...
                "llm_used": True,
                "message": "Proyecto generado con LLM real (async)"
            }
    ...
    async def _generate_code_with_llm(self, project_path: str, plan: dict) -> list:
        """Genera código REAL consultando ASÍNCRONAMENTE al LLM para cada archivo"""
        files_created = []
        
        # 1. package.json - Consultar ASÍNCRONAMENTE al LLM
        package_prompt = f"""
        ...
        package_code = await self.client.generate_code(package_prompt, "Generación de package.json")
        self._write_file(project_path, "package.json", self._clean_json_response(package_code))
        files_created.append("package.json")
        
        # 2. README.md - Consultar ASÍNCRONAMENTE al LLM
        readme_prompt = f"""
        ...
        readme_content = await self.client.generate_code(readme_prompt, "Generación de README")
        self._write_file(project_path, "README.md", readme_content)
        files_created.append("README.md")
        
        # 3. Archivo principal de la aplicación - Consultar ASÍNCRONAMENTE al LLM
        app_prompt = f"""
        ...
        app_content = await self.client.generate_code(app_prompt, "Generación de aplicación principal")
        ...
        for component in components[:3]:  # Limitar a 3 componentes para demo
            task = self._generate_component_with_llm(project_path, component, plan)
            component_tasks.append(task)
        
        # Ejecutar todas las generaciones de componentes en paralelo
        if component_tasks:
        ...
        print(f"✅ Código generado con LLM Async: {len(files_created)} archivos")
        return files_created
    
    async def _generate_component_with_llm(self, project_path: str, component: dict, plan: dict) -> list:
        """Genera un componente específico consultando ASÍNCRONAMENTE al LLM"""
        files = []
        ...
        try:
            component_code = await self.client.generate_code(component_prompt, f"Generación de {component.get('name')}")
            ...
            print(f"   🔧 Componente generado: {file_path}")
