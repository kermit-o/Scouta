...
try:
    ...
    print("✅ Módulos de API importados correctamente")
except ImportError as e:
    print(f"⚠️  Algunos módulos de API no están disponibles: {e}")
    API_MODULES_AVAILABLE = False
    # Crear routers vacíos como fallback
    from fastapi import APIRouter
    ai_analysis = type('MockModule', (), {'router': APIRouter()})()
    auth = type('MockModule', (), {'router': APIRouter()})()
    billing = type('MockModule', (), {'router': APIRouter()})()
    payments = type('MockModule', (), {'router': APIRouter()})()
    projects = type('MockModule', (), {'router': APIRouter()})()
    ui_api = type('MockModule', (), {'router': APIRouter()})()

app = FastAPI(
    title="Forge SaaS API - Complete Version",
    description="API completa para generación de proyectos con IA",
    version="2.0.0"
)
...
# Incluir routers de API si están disponibles
if API_MODULES_AVAILABLE:
    ...
    print("✅ Todos los routers de API montados")
else:
    print("⚠️  Usando routers vacíos para API")
...
async def root():
    return {
        "message": "🚀 Forge SaaS API completa funcionando",
        "version": "2.0.0",
        "ai_agents_loaded": True,
        "available_agents": list(loaded_agents.keys()),
        "api_modules_available": API_MODULES_AVAILABLE
    }
...
async def health():
    return {
        ...
        "api_modules": API_MODULES_AVAILABLE
    }
...
# Endpoint simple de generación de proyectos
...
async def generate_project(request: ProjectRequest, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        project_data = {
            ...
            "agents_used": list(loaded_agents.keys())
        }
        
        projects_db[project_id] = project_data
        
        # Simular proceso en background usando agentes reales
        def simulate_generation(pid):
            try:
                ...
                # Usar agentes reales si están disponibles
        ...
        return ProjectResponse(
            project_id=project_id,
            status="started",
            message=f"Generación de proyecto '{request.project_name}' iniciada con {len(loaded_agents)} agentes",
