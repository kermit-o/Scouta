...
try:
    from api.v1 import projects, ui_api, ai_analysis
    API_ROUTERS_AVAILABLE = True
    print("✅ Routers de API v1 disponibles")
except ImportError as e:
    print(f"⚠️  No se pudieron importar routers de API: {e}")
    API_ROUTERS_AVAILABLE = False

app = FastAPI(title="Forge SaaS API - Enhanced")
...
if API_ROUTERS_AVAILABLE:
    try:
        ...
        app.include_router(ui_api.router, prefix="/api/v1/ui-api", tags=["UI API V1"]) 
        app.include_router(ai_analysis.router, prefix="/api/v1/ai-analysis", tags=["AI Analysis V1"])
        print("✅ Routers de API v1 incluidos")
...
# Endpoints básicos (los que sabemos que funcionan)
@app.get("/")
async def root():
    return {
        ...
        "api_routers_available": API_ROUTERS_AVAILABLE
    }
