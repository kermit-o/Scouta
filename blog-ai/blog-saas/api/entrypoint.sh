...
migrations = [
    ...
    "CREATE TABLE IF NOT EXISTS live_streams (id SERIAL PRIMARY KEY, room_name VARCHAR(100) NOT NULL UNIQUE, title VARCHAR(200) NOT NULL, host_user_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE, status VARCHAR(20) DEFAULT 'live', viewer_count INTEGER DEFAULT 0, max_viewers INTEGER DEFAULT 0, org_id INTEGER DEFAULT 1, description TEXT, started_at TIMESTAMPTZ DEFAULT NOW(), ended_at TIMESTAMPTZ)",
    "CREATE TABLE IF NOT EXISTS live_chat (id SERIAL PRIMARY KEY, room_name VARCHAR(100) NOT NULL, user_id INTEGER REFERENCES users(id) ON DELETE SET NULL, username VARCHAR(100), display_name VARCHAR(100), message TEXT NOT NULL, is_agent BOOLEAN DEFAULT FALSE, agent_name VARCHAR(100), created_at TIMESTAMPTZ DEFAULT NOW())",
    "CREATE TABLE IF NOT EXISTS video_views (id SERIAL PRIMARY KEY, post_id INTEGER NOT NULL REFERENCES posts(id) ON DELETE CASCADE, user_id INTEGER REFERENCES users(id) ON DELETE SET NULL, session_id VARCHAR(64), watch_seconds INTEGER DEFAULT 0, completed BOOLEAN DEFAULT FALSE, created_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ DEFAULT NOW())",
