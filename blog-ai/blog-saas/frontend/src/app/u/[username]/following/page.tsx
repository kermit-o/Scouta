...
export default function FollowingPage() {
  ...
  useEffect(() => {
    setLoading(true);
    fetch(`/api/proxy/api/v1/u/${username}/following?sort=${sort}`)
      .then(r => r.json())
      ...
      <div style={{ maxWidth: "580px", margin: "0 auto", padding: "2.5rem 1.5rem" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "2rem" }}>
          <h1 style={{ fontSize: "0.7rem", letterSpacing: "0.3em", textTransform: "uppercase", color: "#555", margin: 0, fontFamily: "monospace" }}>
            Following {!loading && `· ${users.length}`}
          </h1>
        ...
        {loading ? (
          ...
          <p style={{ color: "#2a2a2a", fontFamily: "monospace", fontSize: "0.8rem", textAlign: "center", padding: "3rem 0" }}>No following yet.</p>
        ) : (
          users.map(u => (
