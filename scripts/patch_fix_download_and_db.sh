...
echo "==> Parchear credenciales de Postgres en docker-compose.yml"
# Fuerza Postgres a usar usuario/clave/db 'forge'
...
# Normaliza DATABASE_URL del backend a forge:forge@postgres:5432/forge
if grep -qE 'DATABASE_URL:.*postgres.*@postgres:5432/(postgres|forge)' docker-compose.yml; then
  sed -i -E 's#(DATABASE_URL:\s*).+#\1postgresql+psycopg2://forge:forge@postgres:5432/forge#' docker-compose.yml
fi

# Además, sustituye cualquier URL dura antigua en compose
sed -i -E 's#postgresql\+psycopg2://postgres:postgres@postgres:5432/postgres#postgresql+psycopg2://forge:forge@postgres:5432/forge#g' docker-compose.yml

echo "==> Actualizar UI para descarga segura del ZIP"
if [ -f ui/app.py ]; then
  cat > ui/app.py <<'PY'
import os
